module.exports = (file, api, options) => {
    ...
    const getBodyStatement = fn => {
        ...
        if (fn.body.type == 'BlockStatement' && fn.body.body.length == 1) {
            ...
            const comments = (fn.body.comments || []).concat(
                inner.comments || []
            );
            ...
            if (
                ...
                const originalLineLength = fn.loc.lines.getLineLength(
                    lineStart
                );
                const approachDifference =
                    'function(a, b) {'.length - '(a, b) => );'.length;
                const argumentLength =
                    inner.argument.end - inner.argument.start;
