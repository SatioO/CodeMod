module.exports = function(file, api, options) {
    ...
    function addNamedExports(exportRef) {
        ...
        if (exportRef.value.declaration.type === 'ObjectExpression') {
            ...
            var replacementExport = j.exportDefaultDeclaration(
                j.identifier(defaultExportName)
            );
            ...
            if (
                exportRef.value.comments &&
                exportRef.value.comments.length > 0
            ) {
        ...
        objectExpressions
            .filter(function(objectExpression) {
                return (
            ...
            .forEach(function(objectExpression) {
                referenceProps = objectExpression.properties.filter(function(
                    prop
                ) {
                    return (
                        'key' in prop &&
                ...
                literalProps = objectExpression.properties.filter(function(
                    prop
                ) {
                    return (
        ...
        var declaredNamedExportDeclaration = createDeclaredNamedExportDeclaration(
            exportRef,
            defaultExportName,
            literalProps
        );
        ...
        var specifiedNamedExportDeclaration = createSpecificedNamedExportDeclaration(
            exportRef,
            defaultExportName,
            referenceProps
        );
    ...
    function createDeclaredNamedExportDeclaration(
        exportRef,
        defaultExportName,
        props
    ) {
    ...
    function createSpecificedNamedExportDeclaration(
        exportRef,
        defaultExportName,
        props
    ) {
        ...
        var specifiers = props
            .filter(function(prop) {
                return shouldCreateNamedExport(
                    defaultExportName,
                    prop.key.name
                );
            })
    ...
    function shouldCreateNamedExport(defaultExportName, exportName) {
        var isAlreadyExportedInDeclaration =
            root
                .find(j.Property, {
                    value: {
                        name: exportName,
        ...
        var isAlreadyExportedAsSpecifier =
