module.exports = function(file, api, options) {
    ...
    const onlyHasSafeClassProperties = path =>
        j(path)
            ...
            .filter(
                p => !(isPropsProperty(p.value) || isStaticProperty(p.value))
            )
            .size() === 0;
    ...
    const needsThisDotProps = path =>
        path
            ...
            .filter(
                p => p.parentPath.parentPath.value.type !== 'MemberExpression'
            )
            .size() > 0;
    ...
    const getAssignmentNames = path => {
        ...
        path.find(j.Identifier)
            .filter(p => {
                ...
                if (
                    !(
                        p.parentPath.value.object &&
                        p.parentPath.value.object.property
                    )
                ) {
                    return true;
                }
        ...
        if (toDestructure) {
            ...
            if (propNames.size > 0) {
                ...
                return j.objectExpression(
                    Array.from(propNames).map(
                        createShorthandProperty(j, typeAnnotation)
                    )
                );
            }
        }
    ...
    const reportSkipped = path => {
        ...
        console.warn(
            `Class "${name}" skipped in ${fileName} on ${line}:${column}`
        );
    };
    ...
    pureClasses.replaceWith(p => {
        ...
        const destructure =
            destructuringEnabled && canDestructure(j(renderMethod));
