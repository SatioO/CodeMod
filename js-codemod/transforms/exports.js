module.exports = function(file, api, options) {
    ...
    function exportsCall(p) {
        var functionCall = j.callExpression(
            p.value.callee.property,
            p.value.arguments
        );
        ...
        filteredPaths.forEach(function(p, i) {
            ...
            if (i === filteredPaths.length - 1) {
                j(p.parentPath).replaceWith(
                    j.exportDeclaration(false, null, specifiers)
                );
            } else {
    ...
    root.find(j.AssignmentExpression, {
        ...
        .filter(function(p) {
            ...
            var isOnBody =
                p.parentPath.parentPath.parentPath.parentPath.name === 'body';
            return isVar && isOnBody;
        })
        .forEach(function(p) {
            var decl = j.variableDeclarator(
                p.parentPath.value.id,
                p.value.right
            );
