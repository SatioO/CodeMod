module.exports = function(file, api) {
    ...
    const getScopeNode = blockScopeNode => {
        ...
        while (TOP_LEVEL_TYPES.indexOf(scopeNode.node.type) === -1) {
            ...
            isInFor =
                isInFor || FOR_STATEMENTS.indexOf(scopeNode.value.type) !== -1;
        }
                ...
                .filter(
                    functionNode =>
                        j(functionNode)
                            .find(j.Identifier)
                            .filter(id =>
                                isIdInDeclarator(declarator, id.value.name)
                            )
                            .size() !== 0
                )
                .size() !== 0;
                    ...
                    const isCalledInHoistedFunction =
                        ...
                        j(scopeNode)
                            ...
                            .filter(n => {
                                return (
                                    n.value.name ===
                                        functionDeclaration.value.id.name &&
                    ...
                    if (
                        ...
                        const isOutsideCurrentScope =
                            j(blockScopeNode)
                                ...
                                .filter(
                                    innerNode =>
                                        innerNode.node.start === n.node.start
                                )
                                .size() === 0;
    ...
    const isMutated = (node, declarator) => {
        ...
        const hasAssignmentMutation =
            j(scopeNode)
                ...
                .filter(n => {
                    return extractNamesFromIdentifierLike(n.value.left).some(
                        name => {
    ...
    const updatedAnything =
        root
            ...
            .forEach(declaration => {
                const forLoopWithoutInit = isForLoopDeclarationWithoutInit(
                    declaration
                );
                if (
